            ...
            <ul className='flex justify-center items-center gap-5 py-4 px-2'>
                {iconSosmed.map((item) => (
                    <li key={item.id} className='flex justify-center items-center w-11 h-11'>
                        <a href={item.href} target='_blank' rel="noopener noreferrer" className='icon-contact relative w-full h-full flex justify-center items-center text-2xl rounded-full border-solid border-2 border-customPrimary transition duration-500 overflow-hidden hover:border-white'>
